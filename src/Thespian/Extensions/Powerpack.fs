...
type Guarantee<'T>(confirmChannel : IReplyChannel<unit>, msg : 'T) = 
    ...
    member g.Payload = 
        if isDisposed then 
            raise 
                (new ObjectDisposedException("Guarantee", "Cannot access the payload of a disposed guaranteed message."))
        msg
...
module ActorRef = 
    ...
    type private MapProtocolClient<'U, 'T>(mapF : 'U -> 'T, client : IProtocolClient<'T>) = 
        ...
        let factory = 
            ...
            | Some clientFactory -> new MapProtocolFactory<'T>(unbox >> mapF, clientFactory) :> IProtocolFactory |> Some
            | None -> None
        
        interface IProtocolClient<'U> with
            member __.ProtocolName = "map" + client.ProtocolName
            member __.ActorId = new MapActorId(client.ActorId) :> ActorId
            member __.Uri = String.Empty
            member __.Factory = factory
            member __.Post(msg : 'U) : unit = client.Post(mapF msg)
            member __.AsyncPost(msg : 'U) : Async<unit> = client.AsyncPost(mapF msg)
            member __.PostWithReply<'R>(msgF : IReplyChannel<'R> -> 'U, timeout : int) : Async<'R> = 
                client.PostWithReply(msgF >> mapF, timeout)
            member __.TryPostWithReply<'R>(msgF : IReplyChannel<'R> -> 'U, timeout : int) : Async<'R option> = 
                client.TryPostWithReply(msgF >> mapF, timeout)
    
    and [<Serializable>] private MapProtocolFactory<'T>(mapF : obj -> 'T, clientFactory : IProtocolFactory) = 
        interface IProtocolFactory with
            member __.ProtocolName = "map" + clientFactory.ProtocolName
            
            member __.CreateClientInstance<'U>(actorName : string) = 
            ...
            member __.CreateServerInstance<'U>(actorName : string, actorRef : ActorRef<'U>) = 
                clientFactory.CreateServerInstance(actorName, actorRef)
...
module Actor = 
    ...
    let reliableBroadcast (actors : #seq<Actor<'T>>) : Actor<Rely<'T>> = 
        ...
        actors
        |> Seq.map (fun a -> a :> Actor)
        |> Actor.bindLinked broadcastBehavior
    ...
    let rec reliablyBroadcastMsg (msgBuilder : IReplyChannel<'R> -> 'T) (actorRefs : ActorRef<'T> list) : Async<'R> = 
        async { 
            ...
            | [] -> 
                return! Async.Raise 
                        <| CommunicationException("Broadcast has failed. Unable to send message to any target.")
            | actorRef :: rest -> 
                try 
    ...
    type private LogSubscribedActor<'T> internal (otherActor : Actor<'T>, observerF : Log -> unit) = 
        ...
        let mutable subscriptionRef : IDisposable option = None
        ...
        override self.Start() = 
            ...
            subscriptionRef <- self.Log
                               |> Observable.subscribe observerF
                               |> Some
            base.Start()
    ...
    let mapMany (mapManyF : 'U -> seq<'T>) (actor : Actor<'T>) : Actor<'U> = 
        Actor.spawnLinked (fun msg -> 
            async { 
                for mappedMsg in mapManyF msg do
                    !actor <-- mappedMsg
            }) [ actor ]
    
    let reliablePostAction (postActionF : Rely<'T> -> Async<unit>) (actor : Actor<Rely<'T>>) : Actor<Rely<'T>> = 
        Actor.spawnLinked (fun (confirmChannel, msg) -> 
            async { 
    ...
    let replicated (replicas : #seq<Actor<'T>>) (actor : Actor<'T>) : Actor<Replicated<'T>> = 
        (replicate replicas actor |> Actor.map (function 
                                         | Replicated msg -> msg
                                         | _ -> failwith "Invalid split"), 
         actor |> Actor.map (function 
                      | Singular msg -> msg
                      | _ -> failwith "Invalid split"))
        |> Actor.split (function 
               | Replicated _ -> true
               | Singular _ -> false)
...
module Failover = 
    ...
    let post, postWithReply = 
            ...
            async { 
                ...
                | candidate :: rest -> 
                    try 
                        ...
                        else let! actors' = candidate <!- refreshMsg
        ...
        (fun refreshMsg logger actors msg -> async { let! _, state' = tryMsg refreshMsg logger true actors 
                                                                      <| Choice1Of2 msg
                                                     return state' }), 
        fun refreshMsg logger actors msgBuilder -> async { let! reply, state' = tryMsg refreshMsg logger true actors 
                                                                                <| Choice2Of2 msgBuilder
            ...
            let rec retryloop = 
                ...
                | n -> 
                    try 
                        self.Post message
                    with
                    | :? TimeoutException | CommunicationException _ -> 
        ...
        member self.PostWithReplyRetriable(msgBuilder, retries, ?waitInterval) = 
            ...
            let rec retryloop n = 
                async { 
                    ...
                    | _ -> 
                        try 
                            return! self.PostWithReply msgBuilder
                        with
                        // bug: actors do not wrap SocketException from client side
                        | :? Sockets.SocketException | :? TimeoutException | CommunicationException _ -> 
                            Thread.Sleep(waitInterval)
