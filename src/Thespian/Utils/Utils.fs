namespace Nessos.Thespian.Utilities
    ...
    module Utils =
        ...
        type Exception with
            ...
            member e.SetStackTrace(trace : string) = remoteStackTraceField.SetValue(e, trace)
        ...
        let inline compareOn (f: 'T -> 'U when 'U : comparison) (x: 'T) (other: obj): int =
            match other with
            | :? 'T as y -> compare (f x) (f y)
            | _ -> invalidArg "other" "Unable to compare values of incompatible types."

        let inline hashOn (f: 'T -> 'U when 'U : equality) (x: 'T): int = hash (f x)

        let inline equalsOn (f: 'T -> 'U when 'U : equality) (x: 'T) (other: obj): bool =
            match other with
            | :? 'T as y -> (f x = f y)
            | _ -> false

        let inline equalsOnComparison (f: 'T -> 'U when 'U : comparison) (x: 'T) (other: obj): bool =
            match other with
