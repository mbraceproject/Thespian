      ...
      protectMailbox (fun mailbox ->
        async {
          ...
          return match reply with
                 | Value value -> value
                 | Exception ex -> raise (new MessageHandlingException("Actor threw exception while handling message.", srv.ActorId, ex))
        })
      ...
      protectMailbox (fun mailbox ->
        async {
          ...
          return match reply with
                 ...
                 | Some(Exception e) -> raise <| new MessageHandlingException("Actor threw exception while handling message.", srv.ActorId, e)
                 | None -> None
        })
