...
type ActorId(actorName: string) =
  ...
  default self.CompareTo(other: ActorId) = self.ToString().CompareTo(other.ToString())
  ...
  interface IComparable<ActorId> with
    override self.CompareTo(otherActorId: ActorId): int = self.CompareTo(otherActorId)
...
type LogSource = Actor of string | Protocol of string    
